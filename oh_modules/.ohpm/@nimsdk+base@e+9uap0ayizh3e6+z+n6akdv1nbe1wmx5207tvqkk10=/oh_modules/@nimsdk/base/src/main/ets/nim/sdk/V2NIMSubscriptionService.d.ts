 ...
 *
 */
export interface V2NIMSubscriptionService extends NIMEBaseServiceInterface<V2NIMSubscribeListener> {
    /**
     ...
     *
     * @param V2NIMCustomUserStatusParams 自定义用户状态
     * @returns V2NIMCustomUserStatusPublishResult
     */
 ...
 * 订阅模块的监听事件
 *
 * 注: 使用 dist/esm 产物时时，需要动态引入 V2NIMSubscriptionService 后使用
 *
 * @example
 * ```
 ...
 * ```
 */
export interface V2NIMSubscribeListener extends NIMEBaseListener {
    /**
     ...
     */
    onUserStatusChanged: [
        userStatusList: V2NIMUserStatus[]
    ];
}
/**
 * 用户状态
