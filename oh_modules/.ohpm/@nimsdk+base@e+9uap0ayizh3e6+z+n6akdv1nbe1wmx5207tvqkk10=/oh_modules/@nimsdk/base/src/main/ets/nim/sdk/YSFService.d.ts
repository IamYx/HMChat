 ...
 * 云商模块
 *
 * 注: 使用 dist/esm 产物时时，需要动态引入 YSFService 后使用
 *
 * @example
 * ```
 * import { YSFService } from '@nimsdk/base'
 * import { YSFServiceImpl } from '@nimsdk/ysf'
 * ```
 */
export interface YSFService extends NIMEBaseServiceInterface<YSFListener> {
    /**
     * ysf 发送消息
     *
 ...
 * 云商模块的事件定义
 */
export interface YSFListener extends NIMEBaseListener {
    /**
     * 本端发送消息状态回调。
     *
    ...
    onReceiveCustomNotifications: [
        notifications: V2NIMCustomNotification[]
    ];
}
