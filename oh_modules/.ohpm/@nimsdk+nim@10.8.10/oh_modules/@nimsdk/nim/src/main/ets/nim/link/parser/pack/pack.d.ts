export declare class Pack {
     ...
     * @param bytes 字节数组
     */
    putRaw(l70: Uint8Array): void;
    /**
     * 将一个 byte 写入缓存
     * @param bt 内容，限定一个 byte 能表达的范围内
     */
    putByte(j70: number): void;
    /**
     * 将一个字符串写入缓存
     * @param str 目标字符串
     */
    putString(g70: string): void;
    /**
     * 将一个整数(4字节)写入缓存
     * @param val 整数内容
     */
    putInt(e70: number): void;
    /**
     * 将一个不定位数的整数写入缓存
     *
     ...
     * @param val 整数内容
     */
    putVarInt(c70: number): void;
    /**
     * 将一个布尔值写入缓存
     *
     * @param val 布尔值
     */
    putBoolean(a70: boolean): void;
    /**
     * 将一个长整数(8字节)写入缓存
     *
     ...
     * @param val 长整数内容
     */
    putLong(x69: number): void;
    putStringAsLong(u69: string): void;
    /**
     * 将一个 short(2字节) 写入缓存
     *
     * 注: 可能后面用不到这个
     ...
     * @param val 数字内容
     */
    putShort(s69: number): void;
    /**
     * 将一个变长的字符串所形成的字节数组写入缓存
     *
     * 注: 按照业务需求 property 数据推入需要先推入长度
     */
    putVarbin(p69?: Uint8Array): void;
}
